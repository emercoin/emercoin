...
namespace {
    ...
    static std::map<uint256, std::pair<NodeId, std::list<QueuedBlock>::iterator> > mapBlocksInFlight GUARDED_BY(cs_main);
...
void static ProcessGetBlockData(CNode* pfrom, const CChainParams& chainparams, const CInv& inv, CConnman* connman)
{
    ...
    if (send && (pindex->nStatus & BLOCK_HAVE_DATA))
    {
        ...
        } else if (0 && inv.type == MSG_WITNESS_BLOCK) {
            // emercoin: We do not use this fast path, sice add to net block extra flags, 4 bytes
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::BLOCK)
    {
        ...
        while (fContinue) {
            ...
            // This ProcessNewBlock can return FALSE for block duplicate, so we do not analyze result here
            // This is old behavior here
...
bool PeerLogicValidation::ProcessMessages(CNode* pfrom, std::atomic<bool>& interruptMsgProc)
{
    ...
    rc4ok_addentropy(*hash.GetDataPtr());
