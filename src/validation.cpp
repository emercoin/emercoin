...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    // emercoin:
    // For SpamBat, we can use 66-bytes transaction (OP_DATA only), so compare to 82 is incorrect.
    unsigned serSize = ::GetSerializeSize(tx, PROTOCOL_VERSION | SERIALIZE_TRANSACTION_NO_WITNESS);
    if (serSize < 66)
    ...
    entry.reset(new CTxMemPoolEntry(ptx, nFees, nAcceptTime, ::ChainActive().Height(),
            fSpendsCoinbase, nSigOpsCost, lp, nAbsurdFee > 0));
    // nAbsurdFee > 0 above -> is_mine, need re-broadcast if not in fresh block, see net_processing.cpp
...
int ApplyTxInUndo(Coin&& undo, CCoinsViewCache& view, const COutPoint& out)
{
    ...
    if (undo.nHeight == 0) {
        ...
        if (!alternate.IsSpent()) {
            ...
            //emcTODO - do we need undo.fCoinStake and undo.nTime? I think, YES. Added. Need check more.
