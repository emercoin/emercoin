...
static UniValue validateaddress(const JSONRPCRequest& request)
{
            ...
            "  \"scriptPubKey\" : \"{json struct}\",(string) scriptPubKey, generate by address: hex and decoded fields\n"
            "}\n"
                },
                RPCExamples{
    ...
    if (isValid)
    {
        ...
        if(pwallet) {
            isminetype mine = IsMine(*pwallet, scriptPubKey);
            ret.pushKV("ismine",      !!(mine & ISMINE_SPENDABLE));
            ret.pushKV("iswatchonly", !!(mine & ISMINE_WATCH_ONLY));
        }
